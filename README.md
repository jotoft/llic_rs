...
This is a Rust port of the LLIC image compression library originally written in C++ by Peter Rundberg. The original C++ implementation is included as a git submodule in the `llic/` directory (from https://gitlab.com/llic/llic.git).

## Overview

LLIC (Lossless/Lossy Image Compression) is an experimental image compression format designed for efficient encoding of grayscale images. This repository contains:
...
Both implementations support multiple quality levels from lossless (Q0) to lossy compression with increasing compression ratios.
...
LLIC uses a custom entropy coding scheme with pre-computed compression tables optimized for image data patterns. The format specification can be found in [LLIC_FORMAT_SPEC.md](LLIC_FORMAT_SPEC.md).
...
The Rust implementation leverages Rayon for parallel processing, while the C++ version uses OpenMP. Both implementations are optimized for multi-core processors.

## Acknowledgements

This project is based on the original LLIC C++ implementation by **Peter Rundberg**. The Rust implementation is a port and enhancement of the original work.
...
The C++ implementation is licensed under the zlib License (see `llic/LICENCE`). The Rust implementation follows the same licensing terms.
...
This is an experimental image compression format under active development. While functional, it may not be suitable for production use without further testing and optimization.
